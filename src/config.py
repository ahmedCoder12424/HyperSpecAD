...
class Config:
    ...
    def __init__(self) -> None:
        ...
        self._parser.add_argument(
            ...
            nargs="+",
            required=True,
            help="One or more MGF files or directories containing MGF files"
        )
        ...
        self._parser.add_argument(
            ...
            help='Discard spectra with fewer than this number of peaks '
                 '(default: %(default)s).')
        self._parser.add_argument(
            '--mz_interval', type=float, default=1,
        ...
        self._parser.add_argument(
            ...
            choices=['louvain', 'dbscan', 'hc_single', 'hc_complete', 'hc_average'],
        ...
        self._parser.add_argument(
            '--anomaly_eps_percentile', type=float, default=0.85,
            help='anomaly_eps_percentile for selecting cut off percentile for anomaly detection Values that work best are 0.8 to 0.95')


        #incremental clustering
        self._parser.add_argument(
            '--init_data',type=str,
            help='name of dataset for initial cluster results')

        self._parser.add_argument(
            '--static_cluster_file',type=str,
            help='name of initial cluster results file')

        self._parser.add_argument(
            '--incr_data',type=str,
            help='name of incr portion of dataset')

        self._parser.add_argument(
            '--incre_eps',type=float,default=0.25,
            help='The eps parameter for DBSCAN incremental clustering, data points incrementally added will use this threashold to determine which cluster to join')
        
        self._parser.add_argument(
            '--incre_ratio',type=float,default=0.6,
            help='the ratio of the dataset that used to generate initial clustering, the rest of data points will join incrementally')

        self._parser.add_argument(
            '--incre_batch_size',type=int,default=10,
            help='the batch size used to update cluster centroids during incremental clustering')

        self._parser.add_argument(
            '--incre_mode',action='store_true',
            help='use incremental clustering or not.')

        self._parser.add_argument(
            '--anomaly_file',type=str,
            help='name of anomaly data file')

        self._parser.add_argument(
            '--anomaly_path',type=str,
            help='path anomaly data file')
