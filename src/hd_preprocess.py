...
def csv_to_parquet(csv_file, parquet_file=None, logger=None):
    """
    Convert a CSV file to a Parquet file.

    Parameters:
        csv_file (str): Path to the input CSV file.
        parquet_file (str): Optional output parquet filename.
                            If None, replaces .csv with .parquet
    ...
    try:
        ...
        if logger:
            logger.info(f"Converted {csv_file} → {parquet_file}")
        else:
        ...
        if logger:
            ...
            print(f"Failed to convert {csv_file} to parquet: {e}")
        return None

def load_metadata(filename, logger):
    ckp_parquet_file = filename
    spectra_meta_df = pd.read_parquet(ckp_parquet_file) \
        if os.path.exists(ckp_parquet_file) else None

    if (spectra_meta_df is not None):
       logger.info("Successfully restored checkpoints from {}!".format(ckp_parquet_file))
    else:
        logger.info("Incomplete checkpoints!")

    return spectra_meta_df

def load_hvs(filename, logger):
    ckp_hvs_file = filename
    ...
    if (spectra_hvs is not None):
        logger.info("Successfully restored checkpoints from {}!".format(ckp_hvs_file))
    else:
        logger.info("Incomplete checkpoints!")
...
def load_datasets(file, mode, logger):
    ...
    ckp_parquet_file = file + '/' + file + '_' + mode + '_meta.ckp'
    ckp_hvs_file = file + '/' + file + '_' +  mode + '_hvs.ckp'
...
def load_data(filename, logger):
    """
    Load spectra metadata and hypervectors from CSV files
    """

    meta_csv = filename + '_meta.csv'
    hvs_csv = filename + '_hvs.csv'

    # Load metadata
    spectra_meta = pd.read_csv(meta_csv)

    # Load hypervectors
    spectra_hvs = pd.read_csv(hvs_csv).values  # convert back to numpy array

    logger.info(f"Loaded spectra metadata from: {meta_csv}")
    logger.info(f"Loaded encoded spectra (hypervectors) from: {hvs_csv}")

    return spectra_meta, spectra_hvs

def load_initial_datasets(config, logger):

    ckp_parquet_file = config.init_data + '_meta' + '.ckp'
    ckp_hvs_file = config.init_data  + '_hvs' + '.ckp'
    print(ckp_parquet_file, ckp_hvs_file)
    spectra_meta_df = pd.read_parquet(ckp_parquet_file) \
        if os.path.exists(ckp_parquet_file) else None
...
def load_incr_data(config, logger):

    ckp_parquet_file = config.incr_data + '_meta' + '.ckp'
    ckp_hvs_file = config.incr_data  + '_hvs' + '.ckp'
    print(ckp_parquet_file, ckp_hvs_file)
    spectra_meta_df = pd.read_parquet(ckp_parquet_file) \
        if os.path.exists(ckp_parquet_file) else None
...
def load_clustering_result(config, logger):

    ckp_parquet_file = config.static_cluster_file+'.parquet'
    print("cluster file name", config.static_cluster_file+'.parquet')
    cluster_results = pd.read_parquet(ckp_parquet_file) \
        if os.path.exists(ckp_parquet_file) else None
...
def save_data_ckp(
    ...
    meta_ckp = filename + '_meta.ckp'
    hvs_ckp = filename + '_hvs.ckp'

    # Save CSV versions
    spectra_meta.to_csv(meta_csv, index=False)
    pd.DataFrame(spectra_hvs).to_csv(hvs_csv, index=False)

    # Save separate checkpoint files
    with open(meta_ckp, "wb") as f:
        pickle.dump(spectra_meta, f)

    with open(hvs_ckp, "wb") as f:
        pickle.dump(spectra_hvs, f)

    logger.info(f"Saved spectra metadata to: {meta_csv}")
    logger.info(f"Saved encoded spectra (hypervectors) to: {hvs_csv}")
    logger.info(f"Saved metadata checkpoint to: {meta_ckp}")
    logger.info(f"Saved hypervector checkpoint to: {hvs_ckp}")
...
def save_data(
    ...
    config : 
        Config that defines runtime parameters
    """
    meta_file = filename + '_meta' + '.csv'
    hvs_file = filename+ '_hvs' + '.csv'

    # Save metadata
    spectra_meta.to_csv(meta_file, index=False)

    # Save hypervectors
    pd.DataFrame(spectra_hvs).to_csv(hvs_file, index=False)

    logger.info(f"Saved spectra metadata to: {meta_file}")
    logger.info(f"Saved encoded spectra (hypervectors) to: {hvs_file}")
...
def export_cluster_results(
    ...
    cluster_csv_filename = config.output_filename

    spectra_df.to_csv(cluster_csv_filename , index=False)
    logger.info("Exporting cluster representatives to {}".format(cluster_csv_filename))
...
def export_anomaly_results(
    ...
    """
    Export anomaly labels to Parquet and CSV.

    Parameters
    ----------
    spectra_df : pd.DataFrame
        Dataframe with anomaly / cluster labels.
    filename : str or PathLike
        Output path prefix (no extension).
    logger : logging.Logger
        Logger instance.
    """
    # Ensure identifier is string (safe for parquet/csv)
    # if "identifier" in spectra_df.columns:
    #     spectra_df["identifier"] = spectra_df["identifier"].astype(str)
    ...
    # Parquet
    spectra_df.to_parquet(parquet_file, compression="snappy", index=False)

    # CSV
    spectra_df.to_csv(csv_file, index=False)

    logger.info(f"Exporting anomaly labels to {parquet_file}")
    logger.info(f"Exporting anomaly labels to {csv_file}")
...
def _precursor_to_interval(
    ...
    # scale = 10#500#1000 # 0.01 Da buckets
    # interval_width = 80

    scale = 10#500#1000 # 0.01 Da buckets
    # interval_width = 0.01

    return int(mz * scale) // interval_width
...
def load_process_spectra_parallel(
    ...
    for input_path in config.input_paths:
        if os.path.isdir(input_path):
            input_files.extend(
                glob.glob(os.path.join(input_path, '*.' + config.file_type))
            )
        elif os.path.isfile(input_path) and input_path.endswith('.' + config.file_type):
