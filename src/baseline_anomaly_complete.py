...
# ── PREPROCESSING CONFIG (m/z-related only) ────────────────────────────────
# Ported from the original Config/argparse defaults. Only the m/z-related
# options are applied in this pipeline; peak-count/intensity/scaling
# options (min_peaks, remove_precursor_tol, min_intensity, max_peaks_used,
# scaling) are NOT applied here.
MIN_MZ       = 0.0   # Minimum peak m/z value (inclusive).
MAX_MZ       = 1500.0  # Maximum peak m/z value (inclusive).
MZ_INTERVAL  = 1.0     # Bin width (Da) used when binning spectra into the feature matrix.
MIN_MZ_RANGE = 5.0   # Discard spectra whose peak m/z range is smaller than this.
...
# ── 2b. M/Z-RELATED PREPROCESSING (min_mz / max_mz / min_mz_range) ──────────

def filter_spectra_by_mz(spectra, min_mz=MIN_MZ, max_mz=MAX_MZ,
                         min_mz_range=MIN_MZ_RANGE):
    """
    Apply the m/z-related preprocessing defaults:
      1. Keep only peaks within [min_mz, max_mz] (inclusive) for every
         spectrum.
      2. Discard spectra whose remaining peak m/z range
         (max(mzs) - min(mzs) among the kept peaks) is smaller than
         min_mz_range.

    Returns a new list of spectra (originals are not mutated) plus a
    print summary of how many spectra were discarded and why.
    """
    kept = []
    n_discarded_empty = 0
    n_discarded_range = 0
    ...
    for s in spectra:
        ...
        mzs_f  = mzs[mask]
        ints_f = intensities[mask]

        if mzs_f.size == 0:
            n_discarded_empty += 1
            continue

        mz_range = float(mzs_f.max() - mzs_f.min())
        if mz_range < min_mz_range:
            n_discarded_range += 1
            continue

        s_filtered                = dict(s)  # shallow copy, don't mutate caller's spectrum
        s_filtered["mzs"]         = mzs_f
        s_filtered["intensities"] = ints_f
        kept.append(s_filtered)

    n_total = len(spectra)
    n_kept  = len(kept)
    print(f"  [m/z filter] min_mz={min_mz}  max_mz={max_mz}  "
          f"min_mz_range={min_mz_range}")
    print(f"  [m/z filter] kept {n_kept:,}/{n_total:,} spectra "
          f"({n_discarded_empty:,} had no peaks in [min_mz, max_mz], "
          f"{n_discarded_range:,} had too narrow a peak m/z range)")
...
def run_pipeline(*paths,
                 anomaly_paths=None,
                 recursive=False,
                 mz_min=MIN_MZ, mz_max=MAX_MZ, bin_width=MZ_INTERVAL,
    ...
    rw.write(f"\nApplying m/z preprocessing "
             f"(min_mz={MIN_MZ}, max_mz={MAX_MZ}, min_mz_range={MIN_MZ_RANGE}) ...")
    with timer.measure("m/z filtering"):
        spectra = filter_spectra_by_mz(spectra, min_mz=MIN_MZ, max_mz=MAX_MZ,
                                       min_mz_range=MIN_MZ_RANGE)

    if not spectra:
        raise RuntimeError("All spectra were discarded by m/z preprocessing filters "
                          "(min_mz/max_mz/min_mz_range) — check paths or thresholds.")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--bin-width",          type=float, default=MZ_INTERVAL)
    parser.add_argument("--mz-min",             type=float, default=MIN_MZ)
    parser.add_argument("--mz-max",             type=float, default=MAX_MZ)
