...
def export_distance_metric(distance_metric, file_name):
   ...
   #logger.info("Exporting distance calculation to distance_metric.csv")
    print("exported distance metric")
# TODO
...
def bundle_async(input_hv, stream):
    with stream:
        input_hv = cp.asarray(input_hv.view(np.uint8))
...
def bundle_1(input_hv):
    ...
    N, byte_len = input_hv.shape
    unpacked_hv = cp.unpackbits(input_hv.ravel())  #unpacked
    unpacked_hv = unpacked_hv.reshape(N, byte_len * 8) #reshaping 
   
   # TBP = 32
    total_columns = byte_len * 8
  # blockspergrid_x = math.ceil(total_columns / TBP)

   #unpacked_hv_result = cp.array(N, byte_len * 8)
    unpacked_bundle = cp.zeros((total_columns,), dtype=cp.uint8)
  # print("finding majority bits")
    threadsperblock = 128
    
    blockspergrid = total_columns
    fast_majority[blockspergrid, threadsperblock](unpacked_hv, unpacked_bundle)
  #  fast_majority(unpacked_hv, unpacked_bundle,  N)
    
  
    cuda.synchronize()
   # print("found majority bits")
    # column_sums = cp.sum(unpacked_hv, axis=0) #summing columns
    # bundled_hv = (column_sums >= (N / 2)).astype(cp.uint8) #choose majority bit 
    bundled_hv = cp.packbits(cp.asarray(unpacked_bundle)) #repack 
    return bundled_hv
...
def fast_majority(A, D):
# sA = cuda.shared.array((TPB,TPB), dtype=nb.uint32)
   #print("beginning majority bit process in cuda kernel")
    smem = cuda.shared.array(shape=(32,), dtype=nb.int32)
    x = cuda.threadIdx.x
    ...
    N, total_columns = A.shape
    s = 0
    if (block_x_index >= total_columns and x >= N):
        return 

    for row in range(x, N, block_dim):
        s += A[row, block_x_index]
...
def get_all_cluster_reps_1(clusters, hvs, bucket=None):
    clusters = np.array(clusters)
    results = []
    spectra_hvs_gpu = cp.asarray(hvs.view(np.uint8))
    
    def worker(cluster):
       # hvs_subset, cluster_subset = #get_cluster_hvs(cluster,cluster_results,hvs_subset)
        indices  = get_cluster_indices(clusters, cluster)
    ...
    with ThreadPoolExecutor() as executor:
        ...
        for future in tqdm(as_completed(futures), total=len(clusters), desc="Bundling clusters"):
...
def get_all_cluster_reps(clusters, hvs, bucket=None):
   ...
   #spectra_hvs_gpu = cp.asarray(hvs.view(np.uint8))
    ...
    #  hvs_subset = hvs[get_cluster_indices(clusters, cluster)]  #get_cluster_hvs(cluster,cluster_results,hvs)
      #  rep_hv = hd_preprocess.bundle(hvs_subset)
      
       #print("found bundle")
#       indices = get_indices(clusters, cluster)
 #       hvs_subset = hvs[indices]
  #      rep_hv = bundle(hvs_subset)

  #  num_streams = min(16, len(clusters)) 
  #  streams = [cp.cuda.Stream() for _ in range(num_streams)]
  #  futures = []

  #  for i,cluster  in enumerate(tqdm(clusters,desc= "bundling clusters")):
   #     stream = streams[i % num_streams] 
    #    indices  = get_cluster_indices(clusters, cluster)
   #     hvs_subset = hvs[indices]
   #     futures.append((cluster, bundle_async(hvs_subset, stream)))

    # synchronize & collect results
  #  reps = []
  #  for (cluster, hv), stream in zip(futures, streams):
   #     stream.synchronize()
    #    reps.append((cluster, cp.asnumpy(hv)))

   # representatives = pd.DataFrame(reps, columns=["cluster", "rep_hv"])
   # return representatives
...
def cluster_bucket(
    ...
    if bucket_slice[1]-bucket_slice[0]==0:
      ...
      # export_distance_metric(pw_dist)
...
def sample_distance(bucket_hv, meta_data, config, file_name):
    bucket_prec_mz  = meta_data['precursor_mz'].values
    pw_dist = fast_nb_cosine_dist_mask(bucket_hv, bucket_prec_mz, config.precursor_tol[0], np.ndarray)
    print(pw_dist)
    export_distance_metric(pw_dist, file_name)

def export_sample_hvs(hv, filename):
    np.savetxt(filename+'.csv', hv, delimiter=',')
    print("saved sample hvs for distance metric")
...
def cluster_spectra(
   ...
   #print("cluster_results type", type(cluster_results))
   # print("CLUSTER_RESULTS", cluster_labels)
    cluster_reps = None  #get_all_cluster_reps(cluster_labels, encoded_spectra_hv)
    ...
    return cluster_labels, cluster_reps, representative_mask
