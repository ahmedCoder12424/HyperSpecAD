...
# df = pd.read_csv("cluster_results_Gr_Si.csv")
df = pd.read_csv("cluster_results_main_mod_2.csv")
...
for size in sizes:
  ...
  df_anomalies = df[df["identifier"].str.contains("anomalies")]

  print(df_anomalies)

  anomaly_cluster = df_anomalies['cluster'].values
  print("anomaly_cluster", anomaly_cluster)
  df_anomalies.to_csv(
      f"inorganic_anomaly_split_files_mod_2/anomaly_spectra_s{size}.csv",
      index=False
  )


  df_non_anomaly = df[~df["cluster"].isin(anomaly_cluster)]
  from sklearn.model_selection import train_test_split

  # count rows per cluster
  cluster_counts = df_non_anomaly['cluster'].value_counts()

  split_sizes = [1, 2, 3, 4,6, 8, 10, 15, 20, 25]
  ...
  for split_size in split_sizes:
    ...
    for cluster in tqdm( df_multi['cluster'].unique(), desc=f"making dataset split for split_size {split_size}"):
    ...
    df_train.to_csv(f"inorganic_anomaly_split_files_mod_2/train_datak{split_size}.csv", index=False)

    excluded_protein = pd.read_csv(f"inorganic_anomaly_split_files_mod_2/anomaly_spectra_s{size}.csv")
    ...
    print("number of anomalies in test data", len(df_test[df_test["cluster"].isin(anomaly_cluster)]))

    df_test.to_csv(f"inorganic_anomaly_split_files_mod_2/test_data-mixedk{split_size}.csv", index=False)
