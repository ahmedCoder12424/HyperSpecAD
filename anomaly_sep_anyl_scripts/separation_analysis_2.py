...
print("len of cluster_results", len(cluster_results))
print("# of clusters", len(cluster_results['cluster'].unique()))
sys.exit(0)
...
for cluster_id in tqdm(unique_clusters, desc="Computing separation"):
    ...
    if len(candidate_clusters) == 0:
       ...
       #CHANGE THIS BACK IMMEDIATELY
        ...
        all_hvs = np.vstack([rep_hv[None, :], other_hvs])
        all_mzs = np.concatenate([[rep_mz], other_mzs]).astype(np.float32).reshape(-1, 1)

        dist_mat = hd_cluster.fast_nb_cosine_dist_mask(all_hvs, all_mzs, 20, "numpy")
        # print("distance metric")
        # print(dist_mat)
        inter_dists = dist_mat[0, 1:]
        # print("intern dists")
        # print(inter_dists)

  

        best_i = int(np.argmin(inter_dists))
        nearest_cluster_dist = float(inter_dists[best_i])
...
cluster_stats_df.to_csv("cluster_separation_stats_by_bucket_new.csv", index=False)

print("Saved cluster_separation_stats_by_bucket_new.csv")