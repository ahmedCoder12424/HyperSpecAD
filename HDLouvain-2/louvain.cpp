...
int values=0;
int aggregate_graph(Louvain_Partition& p, bool free_memory = true) 
{
   ...
   */
    std::vector<int> renumber(p.l, -1);
    std::vector<int> count(p.l + 1, 0);
    std::vector<int> reorder(p.l);
    for (int i = 0; i < p.l; i++) {
        renumber[i] = -1;
        reorder[i]=p.nc[i];
...
//out << "starting louvain_main" << end;
//out << "starting louvain_main" << endl;
double quality=0.0;
double newGain=0.0;
...
for (auto it = p.labels.begin(); it != p.labels.end(); ) {
    auto &vec = it->second;
    if (!vec.empty() && p.real_id[p.nc[index]] != it->first) {
        auto &target = p.labels[p.real_id[p.nc[index]]];
